...
def evaluate(estimate, openpose):
    ...
    for i in range(n_frames):
        ...
        image_path = f"../NGT_Temporary_4/sample14/cropped_front/resized_{openpose['img_path'][i]}"
        image_to_viz = cv2.imread(image_path)
        for coord in est_rhand_crop.cpu().numpy():
            cv2.circle(image_to_viz, (int(coord[0] * 768), int(coord[1] * 768)), 5, (0, 255, 0), -1)
        for coord in op_rhand_t.cpu().numpy():
            cv2.circle(image_to_viz, (int(coord[0] * 768), int(coord[1] * 768)), 5, (0, 0, 255), -1)
        cv2.imwrite(f"frame_{i}_viz.png", image_to_viz)

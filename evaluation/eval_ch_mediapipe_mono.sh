...
echo "Batch,Sample,Opt_LHand_MSE,Opt_RHand_MSE" > "$SAMPLE_RESULTS_FILE"
echo "Batch,Total_Samples,Avg_Opt_LHand,Avg_Opt_RHand" > "$BATCH_AVERAGES_FILE"
...
for batch in {1..9}; do
    ...
    for sample_dir in "$batch_dir"/sample*; do
        ...
        # Check if required files exist (base_file is still needed as ground truth target for metric comparison)
        if [[ -f "$opt_file" && -f "$base_file" ]]; then
            ...
            # SAFETY CHECK: Ensure the python script didn't crash
            if [[ -z "$o_lhand" ]]; then
            ...
            echo "${batch},${sample_name},${o_lhand},${o_rhand}" >> "$SAMPLE_RESULTS_FILE"
    ...
    if [ "$batch_samples" -gt 0 ]; then
        ...
        echo "${batch},${batch_samples},${avg_o_lhand},${avg_o_rhand}" >> "$BATCH_AVERAGES_FILE"
...
if [ "$overall_samples" -gt 0 ]; then
    ...
    echo "----------------------------------------------------------"
    echo "METRIC         | OPTIMIZATION AVG "
    echo "----------------------------------------------------------"
    echo "Left Hand MSE  | $oa_o_lhand"
    echo "Right Hand MSE | $oa_o_rhand"
else

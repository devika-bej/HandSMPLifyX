...
def load_data(data, image_folder, ind):
...
def render_model(renderer, model_output, img, outdir, ind, file_name_suffix=""):
    ...
    overlay_file_name = os.path.join(outdir, f"{ind}_{file_name_suffix}.png")
...
def main():
    ...
    n = len(full_data['imgname'])

    for ind in range(n):
        # Load data from npz
        data = load_data(full_data, IMAGE_FOLDER, ind)
        ...
        render_model(renderer, model_output, img, OUTPUT_DIR, ind, file_name_suffix="overlay")

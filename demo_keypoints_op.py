...
def extract_valid_pose(kp_data, w, h):
    ...
    if kp_data is not None and len(kp_data.shape) > 0 and kp_data.shape[0] > 0:
        pose = np.array(kp_data[0][:, :2]) / [w, h]
        return pose * [768, 768]
    return None

def fill_missing_poses(pose_list):
    """
    Fills missing (None) poses. 
    ...
    # Pass 1: Forward fill (take from previous)
    for i in range(1, n):
        if pose_list[i] is None and pose_list[i-1] is not None:
            pose_list[i] = pose_list[i-1]

    # Pass 2: Backward fill (take from next - handles missing initial frames)
    for i in range(n - 2, -1, -1):
        if pose_list[i] is None and pose_list[i+1] is not None:
            pose_list[i] = pose_list[i+1]

    # Pass 3: Fallback if the body part was NEVER detected in the whole sequence
    for i in range(n):
        if pose_list[i] is None:
            pose_list[i] = np.empty((0, 2))

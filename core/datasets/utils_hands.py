...
def get_transform(center, scale, res, rot=0):
    ...
    h = 200 * scale
    t = np.zeros((3, 3))
    t[0, 0] = float(res[1]) / h[0]
    t[1, 1] = float(res[0]) / h[1]
...
def get_example(img_path: str|np.ndarray, center_x: float, center_y: float,
    ...
    # img_patch_cv_lh, trans__ = generate_image_patch_cv2(cvimg,
    #                                                 center_lx, center_ly,
    #                                                 lh_width, lh_width,
    #                                                 patch_width, patch_height,
    #                                                 do_flip, scale, rot, 
    #                                                 border_mode=border_mode)
    # import time
    # cv2.imwrite(str(int(time.time()))+'temp.png', img_patch_cv)

    image = img_patch_cv.copy()
    trans_lhand = gen_trans_from_patch_cv(center_lx, center_ly, lh_width, lh_width, patch_width, patch_height, scale, rot)
    trans_rhand = gen_trans_from_patch_cv(center_rx, center_ry, rh_width, rh_width, patch_width, patch_height, scale, rot)
    ...
    if not return_trans:
        ...
        return img_patch, img_patch_cv, keypoints_2d, img_size, center_x, center_y,trans_lhand,trans_rhand,width, height, trans, scale
